...
SceneTrainingSC::SceneTrainingSC(){
    R = -1;
    C = -1;
    COUNT = -1;
    death_count = 0;
...
void SceneTrainingSC::init(int r, int c, int count, int s_w, int s_h, int depth, SDL_Renderer* rend, std::vector<int> seq, PieceRandomizer pr, int gen){
    generation = gen;
...
void SceneTrainingSC::render(SDL_Renderer* rend){
...
void SceneTrainingSC::end_game(){
    for(int i = 0; i < mother_genetic_sequence.size(); i++){
        std::cout<<mother_genetic_sequence[i]<<" ";
    }
    // for(int i = 0; i < father_genetic_sequence.size(); i++){
    //     std::cout<<father_genetic_sequence[i]<<" ";
    // }
    ...
    // AIGAME_L[COUNT-1].set_genetic_sequence(mother_genetic_sequence);
...
void SceneTrainingSC::update(SDL_Renderer* rend, int dt, SDL_Event* event, bool* isRunning, int* state){
    ...
    if(line_timer <= 0){
        line_timer = line_timer_dur;
        // for(int i = 0; i < COUNT; i++){
        //     AIGAME_L[i].on_enemy_line_sent(1);
        // }
    }
    ...
    if(end_timer <= 0){
        ...
        int second_max_score_index = -1;
        for(int i = 0; i < COUNT; i++){
            if(AIGAME_L[i].lifetime_lines_sent > max_score){
                second_max_score = max_score;
                second_max_score_index = max_score_index;
                max_score = AIGAME_L[i].lifetime_lines_sent;
                max_score_index = i;
            } else if(AIGAME_L[i].lifetime_lines_sent > second_max_score){
                second_max_score = AIGAME_L[i].lifetime_lines_sent;
                second_max_score_index = i;
            }
        }

        mother_genetic_sequence = AIGAME_L[max_score_index].genetic_sequence;
        father_genetic_sequence = AIGAME_L[second_max_score_index].genetic_sequence;
